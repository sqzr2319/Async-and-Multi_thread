    ...
    public async Task UpdateInventoryAsync(string title, int quantity)
    {
        ...
        lock (_lock)
        {
            ...
            if (book.Inventory >= quantity)
            {
                ...
                Console.WriteLine($"购买成功：{title} x{quantity}，剩余库存：{book.Inventory}");
            }
            else
            {
                Console.WriteLine($"库存不足：{title}，请求数量 {quantity}，当前库存 {book.Inventory}");
            }
        }
    }
}
